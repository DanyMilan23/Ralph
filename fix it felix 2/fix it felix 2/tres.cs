...
namespace fix_it_felix_2
{
    class tres
    {
        ...
        Bitmap todo = new Bitmap(Properties.Resources.Nivel_3.Width, Properties.Resources.Nivel_3.Height);
        bool jugable;
        ...
        public tres(int vi)
        {
            jug = new jugabilidad("Difícil",vi);
            ; jug.Fel.X = 400; jug.Fel.Y = 340;
            generar_ventanas();
            cargar_jugador();
            fixit = true;
            Jugable = false;
        ...
        public void dibujar_ventanas()
        {
            ...
            todo = new Bitmap(Properties.Resources.Nivel_3);
            int fila = 1, columna = 0;
            Graphics grfo = Graphics.FromImage(todo);
            for (int i = 0; i < Vidrio.Length; i++)
            {
